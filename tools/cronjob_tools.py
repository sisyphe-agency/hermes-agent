    ...
    "parameters": {
        ...
        "properties": {
            ...
            "use_memory": {
                ...
                "default": False,
                "description": (
                    "Default: False (memory-isolated — the cron run neither recalls nor writes external memory). "
                    "Set True to let this cron run participate in the profile's external memory (Honcho): it recalls prior context before running and writes the run back afterwards. "
                    "Writes are scoped to a dedicated ``<profile>-cron`` peer, so the run NEVER pollutes the human user's memory representation. "
                    "Use True for jobs that benefit from cross-run continuity (e.g. an evolving daily digest that should remember what it already reported); leave False for stateless watchdogs and one-shot alerts."
                ),
            },
        },
        "required": ["action"]
